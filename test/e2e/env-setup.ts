 ...
 *
 * Loads .env via dotenv/config, then overrides DATABASE_URL to point to
 * the test database by appending "_test" to the dev database name.
 * This must happen before env.validation.ts runs (which is triggered
...
if (!process.env.DATABASE_URL) {
  throw new Error('DATABASE_URL is not set. Ensure .env exists with a valid DATABASE_URL.');
}
